...
from pylode.kvec_generator import Kvector_Generator
from pylode.spherical_harmonics import evaluate_spherical_harmonics
from pylode.radial_projection import innerprod, radial_projection_gto
...
class TestKvecgen:
    ...
    eps = 1e-2
    cutoffs = [1+eps, np.sqrt(2)-eps, np.sqrt(2)+eps, np.sqrt(3)-eps, 
                    np.sqrt(3)+eps, 2-eps, np.sqrt(5)-eps]
    ...
    def test_radial_projection(self):
        ...
        for i in range(self.lmax):
            for j in range(i,self.lmax):
                ...
                exponent = i+j+2
                assert abs(monomialprod - self.rcut**(exponent+1)/(exponent+1)) < 1e-5
